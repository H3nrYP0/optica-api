...
def get_citas():
    try:
        ...
        pagination = Cita.query.order_by(Cita.id.desc()).paginate(page=page, per_page=per_page, error_out=False)
        return jsonify({
    ...
    try:
        ...
        if nuevo_servicio_id != cita.servicio_id:
            ...
            if not servicio.estado:
                return jsonify({"error": "El servicio no está activo"}), 400
            duracion = servicio.duracion_min
        if (nuevo_empleado_id != cita.empleado_id or nueva_fecha_str is not None or nueva_hora_str is not None or nuevo_servicio_id != cita.servicio_id):
            validacion = validar_disponibilidad_cita(
    ...
    try:
        ...
        if 'estado' in data:
            if not data['estado'] and servicio.citas:
                citas_pendientes = [c for c in servicio.citas if c.estado_cita_id == 1]
                if citas_pendientes:
...
def create_horario():
    try:
        ...
        if not empleado or not empleado.estado:
            return jsonify({"error": "Empleado no válido o inactivo"}), 400
        horario_existente = Horario.query.filter_by(empleado_id=data['empleado_id'], dia=data['dia'], activo=True).first()
...
def verificar_disponibilidad():
    try:
        ...
        validacion = validar_disponibilidad_cita(empleado_id, fecha_date, hora_time, duracion, exclude_cita_id)
...
def verificar_disponibilidad_multiple():
    # Este endpoint es público (para landing)
    try:
        servicio_id = request.args.get('servicio_id', type=int)
        fecha_str = request.args.get('fecha')
