...
from app.models import Usuario, Rol, Cliente, Permiso
...
def get_usuario_por_email(email):
    try:
        ...
        if not usuario:
            return jsonify({"success": False, "error": "Usuario no encontrado"}), 404
        return jsonify({"success": True, "usuario": {"id": usuario.id, "nombre": usuario.nombre, "correo": usuario.correo, "rol_id": usuario.rol_id, "cliente_id": usuario.cliente_id}})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
