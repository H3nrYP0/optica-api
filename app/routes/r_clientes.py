"""
Módulo de clientes: CRUD administrativo, perfil público, citas de clientes.
Permisos granulares:
- /admin/clientes: ver_clientes, crear_clientes, editar_clientes, eliminar_clientes
- /cliente/perfil y /cliente/citas: jwt_requerido (sin permiso adicional)
- Rutas públicas (sin /admin): sin autenticación
"""

from flask import jsonify, request
from app.database import db
from app.Models.models import Cliente, HistorialFormula, Cita, Usuario, Empleado, Servicio, EstadoCita
from app.auth.decorators import permiso_requerido, jwt_requerido, get_usuario_actual
...
def create_historial_formula():
    try:
        ...
        historial = HistorialFormula(
            ...
            od_esfera=data.get('od_esfera'), od_cilindro=data.get('od_cilindro'), od_eje=data.get('od_eje'),
            oi_esfera=data.get('oi_esfera'), oi_cilindro=data.get('oi_cilindro'), oi_eje=data.get('oi_eje')
        )
...
def cancelar_mi_cita(cita_id):
    try:
        ...
        ahora = datetime.utcnow()
        if datetime.combine(cita.fecha, cita.hora) < ahora:
            return jsonify({"error": "No se puede cancelar una cita que ya pasó"}), 400
        cita.estado_cita_id = 4
        db.session.commit()
        return jsonify({"success": True, "message": "Cita cancelada correctamente", "cita": cita.to_dict()})
    except Exception as e:
        db.session.rollback()
